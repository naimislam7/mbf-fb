...
class ngentod:
	...
	def menu(self):
		...
		if len(self.id)!=0:
			print("\n\n[?] Pilih Metode Crack")
			print("[1] Crack Dari Api Fb (Mode Crack Cepat:v)")
			print("[2] Crack Dari mbasic (Mode Crack Lumayan Cepat:v)")
			self.ngontol()
		else:
	...
	def login(self):
		try:
			...
			if "mbasic_logout_button" in cek:
				...
				aapganteng_(self.kuki.get("cookie")).myinfo()
				a=awokawokawok(self.kuki.get("cookie"));a.reaksi();open("cookie","w").write(self.kuki.get("cookie"));exit("[✓] Login Berhasil, Jalankan Ulang Tools Nya")
			else:
				exit("\n\n[!] Cookie Invalid")
		except req.exceptions.ConnectionError:
			exit("[!] Kesalahan Pada Koneksi")
